...
默认制作固定构图的连续分镜式动态漫画。V0.4 是已验收的执行引擎；V1.0 在其上增加项目状态识别、入口分流、最小提问、统一 Scene 路由、用户反馈定位和局部返工控制。完整总控规则见 [V1.0 总控系统](references/v10-control-system.md)，执行闭环见 [V0.4 制作闭环](references/v04-production.md)。V1.0 仍不是精确音素口型、自动抠图或无人审核的全自动成片。
