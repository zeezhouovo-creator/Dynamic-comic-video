...
- 默认生图优先使用 `render_profile: "comic-economy"`：角色身份和动作可读性优先，背景适度简化，避免为每个镜头生成不必要的精致材质。详细规则见 [漫画经济画面档位](references/comic-generation-economy.md)。
...
- 每镜以动作与表情变化为核心。当前 V0.4 使用事件驱动的局部关键帧、对齐姿态图片切换和配音驱动的简单嘴部开合；视差只属于旧兼容路径。除眼睛和嘴巴外，也可以对头部、手部、手臂、道具或界面做一次性局部动作，但必须有准备→动作→回稳阶段，并通过固定相机回归夹具验证接触点和遮挡。详见 [有限动画](references/limited-animation.md)。不包含自动补间、精确音素口型或骨骼绑定。
