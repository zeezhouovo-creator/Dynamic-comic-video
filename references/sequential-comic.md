...
独立漫画分镜是最小叙事单位，不追求传统动画的连续运动。分镜和 master 由 Skill 根据 brief 生成；用户明确授权复用且通过尺寸、身份、空间和分层检查的已有分镜可以进入最终镜头，单纯参考图不能自动复用。先拆 Narrative Beats，再拆一句对白、一个简单动作或一个反应；不要机械地每句话固定时长。复杂动作、姿态转换、新视角优先另绘下一张漫画分镜并硬切。
